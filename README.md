...
-----------------------
Building the dependencies and HellenicCoin Core requires some essential build tools and libraries to be installed before.
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, HellenicCoin Core should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, HellenicCoin Core may be compiled in
...
Add `CC=egcc CXX=eg++ CPP=ecpp` to the dependencies build and the HellenicCoin Core build:
```bash
$ cd depends

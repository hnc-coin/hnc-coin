...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a HellenicCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and helleniccoin: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about HellenicCoin Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your HellenicCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified HellenicCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a helleniccoin: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible HellenicCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to HellenicCoin network", "", count));
    } else {
...
{
    QString strTitle = tr("HellenicCoin Core"); // default title
    ...
    // Append title to "HellenicCoin Core - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;

...
const char * const BITCOIN_CONF_FILENAME = "helleniccoin.conf";
const char * const BITCOIN_PID_FILENAME = "helleniccoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "helleniccoin" is a composite category enabling all HellenicCoin-related debug output
                if(ptrCategory->count(std::string("helleniccoin"))) {
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HellenicCoin";
#else
    // Unix
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty helleniccoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override helleniccoin.conf

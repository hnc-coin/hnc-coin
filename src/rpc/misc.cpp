...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total helleniccoin balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized helleniccoin balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "                       mempoolrej, net, proxy, prune, qt, rand, reindex, rpc, selectcoins, tor, zmq, helleniccoin\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "helleniccoin")
            + HelpExampleRpc("debug", "helleniccoin+net")
        );
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given helleniccoin address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The helleniccoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The helleniccoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: HellenicCoin address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are helleniccoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) helleniccoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The helleniccoin address to use for the signature.\n"
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"satoshis\"  (number) The difference of Hellenic\n"
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"satoshis\"  (number) The number of Hellenic of the output\n"
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"balance\"  (string) The current balance in Hellenic\n"
            "  \"received\"  (string) The total number of Hellenic received (including change)\n"
            "}\n"
            "\nExamples:\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* HellenicCoin features */
    { "helleniccoin",               "mnsync",                 &mnsync,                 true,  {} },
    { "helleniccoin",               "spork",                  &spork,                  true,  {"value"} },

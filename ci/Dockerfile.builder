...
# helleniccoin_hash
RUN git clone https://github.com/helleniccoinpay/helleniccoin_hash
RUN cd helleniccoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} helleniccoin
RUN useradd -u ${USER_ID} -g helleniccoin -s /bin/bash -m -d /helleniccoin helleniccoin
...
RUN mkdir /helleniccoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /helleniccoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /helleniccoin-src

...
export BUILDER_IMAGE_NAME="helleniccoin-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/helleniccoin"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_HNC_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true

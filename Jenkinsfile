...
// which are part of the HellenicCoin repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="helleniccoin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /helleniccoin-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "helleniccoinpay-helleniccoin/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from helleniccoinpay-helleniccoin/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'helleniccoinpay-helleniccoin/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from helleniccoinpay-helleniccoin/develop"
            }
          }

          if (hasCache) {
            sh "cd /helleniccoin-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /helleniccoin-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /helleniccoin-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /helleniccoin-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /helleniccoin-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /helleniccoin-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /helleniccoin-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }

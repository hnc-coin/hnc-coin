...
*Setup instructions for a Gitian build of HellenicCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the HellenicCoin
...
- [Building HellenicCoin Core](#building-helleniccoin-core)
...
Clone the git repositories for HellenicCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/helleniccoinpay/helleniccoin
...
in the HellenicCoin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building HellenicCoin Core
----------------

To build HellenicCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the HellenicCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/helleniccoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/helleniccoinpay/helleniccoin
...
For example:
```bash
URL=https://github.com/crowning-/helleniccoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit helleniccoin=${COMMIT} --url helleniccoin=${URL} ../helleniccoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit helleniccoin=${COMMIT} --url helleniccoin=${URL} ../helleniccoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit helleniccoin=${COMMIT} --url helleniccoin=${URL} ../helleniccoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the helleniccoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../helleniccoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/helleniccoin
SIGPATH=/some/root/path/helleniccoin-detached-sigs

./bin/gbuild --url helleniccoin=${BTCPATH},signature=${SIGPATH} ../helleniccoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/helleniccoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/helleniccoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/helleniccoin-osx-build.assert
```
...
[helleniccoin/gitian.sigs](https://github.com/helleniccoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.

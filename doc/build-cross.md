Cross-compiliation of HellenicCoin Core
===============================

HellenicCoin Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building HellenicCoin.
...
When building HellenicCoin Core, use
